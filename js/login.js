...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleLogin() {
    ...
    try {
      const res  = await fetch(`${GEO_API}/auth/login`, {
        method:  'POST',
      ...
      GeoAuth.setToken(json.data.token);
      GeoAuth.setCurrentUser(json.data.user);
      if (rememberMe) GeoAuth.saveRememberedCredentials(role, identifier);
      else            GeoAuth.clearRememberedCredentials();

      loginBtn.innerHTML = `<span class="btn-text">Welcome, ${json.data.user.fullName.split(" ")[0]}!</span>`;
      setTimeout(() => redirectByRole(role), 800);
    } catch (_) {
      showError("Could not reach the server. Please try again shortly.");
      setLoading(false);
    }
  }
  ...
  function redirectByRole(role) {
    const paths = {
      ...
      computer: "student-dashboard2.html"  /* Computer students use the student portal */
    };
    window.location.href = paths[role] || "login2.html";
  }
