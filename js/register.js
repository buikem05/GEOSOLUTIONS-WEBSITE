...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleRegister() {
    ...
    if (role === "student") {
      ...
      if (!subject) { showError("Please enter your subject."); return; }
    } else if (role === "admin") {
      ...
      identifier = document.getElementById("program")?.value.trim() || email;
    }

    setRegLoading(true);

    try {
      const res  = await fetch(`${GEO_API}/auth/register`, {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ fullName, email, password, role, identifier, subject, phone })
      });
  ...
  function setRegLoading(loading) {
    ...
    btn.innerHTML = loading
      ? `<span class="btn-loader"></span><span class="btn-text">Creating Account...</span>`
