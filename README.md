...
The production portal is wired to the Node/Express API in `server/`.

1. Copy `server/.env.example` to `server/.env`.
2. Fill in the database, JWT, admin registration, Paystack, and CORS values.
3. Import `php/schema/geo_academy.sql` into MySQL.
4. Run the API:
...
Required environment variables: `DB_HOST`, `DB_USER`, `DB_NAME`, and `JWT_SECRET`.
The API will refuse to start without them.

The browser login/register flow no longer uses localStorage password fallback. The server must be running for portal authentication.
